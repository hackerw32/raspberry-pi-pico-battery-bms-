...
class I2CResponder:
    ...
    def __init__(self, i2c_device_id=0, sda_gpio=0, scl_gpio=1, responder_address=0x41):
        ...
        # Clear 10 Bit addressing bit, CONTROLLER bit, RESPONDER_DISABLE bit
        self.clr_reg(
            self.IC_CON,
            (
    ...
    def put_read_data(self, data):
        ...
        # ΔΙΟΡΘΩΣΗ: Υποστήριξη για αποστολή λίστας ή bytes 
        if type(data) in (list, tuple, bytes, bytearray):
            for b in data:
                mem32[self.i2c_base | self.IC_DATA_CMD] = b & 0xFF
        else:
